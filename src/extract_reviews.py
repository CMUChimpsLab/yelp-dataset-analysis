...
business_df = business_df[business_df.business_id == 'Q4ULAiIE2qCA4RYplCTrUQ']
...
# # print review_df.groupby('business_id').size().sort_values(ascending=False)[:5]
#
# review_df = review_df[review_df.business_id.isin(business_df.business_id)]
# print review_df.head()
# print review_df.info()
# print review_df.groupby('business_id').size().sort_values(ascending=False)[:15]
#
# # count = 1
# time = 0
# l = 0;
# s = review_df.groupby('business_id').size().sort_values(ascending=False)[:15]
...
#     print ' id', ind
#     print ind, val
#     df_temp = review_df[review_df.business_id == ind]
#     for index, row in df_temp.iterrows():
#         time = time +1;
#         texts.append(row['text'])
#     with open('review'+ str(count) +'.txt', 'w') as f:
#         for s in texts:
#             s = s.encode('ascii', 'ignore').decode('ascii')
#             f.write(s + '\n')
#             f.write((s + u'\n').encode('unicode-escape'))
#         print 'review'+ str(count) +'.txt' +'done'



# texts = []
# for index, row in review_df.iterrows():
#     if row['business_id' == 'SsGNAc9U-aKPZccnaDtFkA']:
#         texts.append(row['text'])
#
# print texts[:10]
